...
function(_mexiface_make_matlab_targets)
    ...
    if(niter GREATER_EQUAL 0)
        foreach(idx RANGE ${niter})
            ...
            message("[MexIFace::Matlab] Making targets for root:${_root} vers:${_vers} ver str:${_ver_string}")
            ...
            if(UNIX)
                set(extern_lib_dir ${_root}/extern/lib/glnxa64)
            ...
            set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_INCLUDE_DIRECTORIES ${include_dir})
            set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_COMPILE_OPTIONS -fexceptions)
            set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_COMPILE_OPTIONS -fno-omit-frame-pointer)
            set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_COMPILE_DEFINITIONS MATLAB_MEX_FILE)
            set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_LINK_LIBRARIES Pthread::Pthreads)
            set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_LINK_OPTIONS "-Wl,--no-undefined -Wl,--as-needed")
            if(UNIX)
                set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_LINK_OPTIONS "-Wl,--version-script,${mapfile}") #Declares visibility of symbols.
            endif()
            set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_LINK_DIRECTORIES "${lib_dir};${extern_lib_dir}")
            ...
            if(MexIFace_MATLAB_INTERLEAVED_COMPLEX AND ${_vers} VERSION_GREATER_EQUAL 9.4)
                set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_COMPILE_DEFINITIONS MATLAB_DEFAULT_RELEASE=R2018a)
            else()
                set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_COMPILE_DEFINITIONS MATLAB_DEFAULT_RELEASE=R2017b)
            endif()
            #Support for 64-bit indexed arrays
            if(MexIFace_MATLAB_LARGE_ARRAY_DIMS AND ${_vers} VERSION_GREATER_EQUAL 9.2)
                set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_COMPILE_DEFINITIONS MX_COMPAT_64)
            else()
                set_target_properties(${target_prefix}MEX_LIBRARIES PROPERTIES INTERFACE_COMPILE_DEFINITIONS MX_COMPAT_32)
            endif()
